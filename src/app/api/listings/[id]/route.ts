...
export async function GET(
    ...
    try {
        const db = readDb();
        const item = [...db.properties, ...db.lifestyle].find(i => i.id === params.id);
...
export async function PUT(
    ...
    try {
        ...
        const propIndex = db.properties.findIndex((i: any) => i.id === params.id);
        if (propIndex !== -1) {
        ...
        if (!found) {
            const lifeIndex = db.lifestyle.findIndex((i: any) => i.id === params.id);
            if (lifeIndex !== -1) {
...
export async function DELETE(
    ...
    try {
        ...
        db.properties = db.properties.filter((i: any) => i.id !== params.id);

        const initialLifeCount = db.lifestyle.length;
        db.lifestyle = db.lifestyle.filter((i: any) => i.id !== params.id);
